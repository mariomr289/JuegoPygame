...
class ReadObj(object):
    ...
    def _to_canonview(self):
        bb = [map(min, zip(*self.vertices)), map(max, zip(*self.vertices))]
...
class MaterialData(object):
    def __init__(self, obj, materialList, minindex=0, maxindex=FLOAT_INF):
        ...
        self.tex_coords = obj.tex_coords
        self.normals = obj.normals
        self.face_groups = {k:v  for k,v in obj.face_groups.iteritems() if k in materialList}                     
        self.materials = {k:v  for k,v in obj.materials.iteritems() if k in materialList}
        self.vbo_dic = {}
